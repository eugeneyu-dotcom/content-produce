...
4. **不要靠讀幾個 organic 結果目測「競爭度中低」(2026-09-04 修正)**:改用
   `scripts/kd_score.js` 算出實際的 KD 分數,不要心算——同一批關鍵字每次判斷都可能因為
   當下的感覺不同而得出不同結論,這正是 oeil-bleu-grec 這類反覆重寫還是沒起色的案例裡,
   當初「這個應該可以打」的判斷本身就沒有數字支撐的問題:
   ```bash
   node .claude/skills/content-farm-writer/scripts/kd_score.js --live --site <站名> --keyword "<候選關鍵字>"
   ```
   站名用 `joaillerie`/`Dream`/`Desk`/`Legend`,腳本會自動用對應站台的語言/地區呼叫
   Serper、抽出前 10 名網域,用該站自己維護的網域分層清單(`SITE_TIERS`,分 authority/minor
   兩層,起始清單見腳本本體,查到新的常見網域直接補進去)算出 0-100 的 KD 分數:
   **KD ≤60(低/中)** 才是這一輪優先處理的對象、**KD >60(高)** 代表需要長期布局或直接跳過,
   不要單看幾個網域名字就下結論。**這支腳本目前只回答「這個字打不打得進前十」,不回答
   「這個字有沒有人搜」**——真實搜尋量要等 Google Ads/GKP 帳號申請下來後才能補上,這是
   已知的資料缺口,不是腳本沒做完,四象限判斷完整版(搜尋量×KD)要等那之後才能做。
5. **當地語言的關鍵字絕對不能用翻譯取得(2026-09-04 新增)**:翻譯(不管誰翻、翻得多道地)
   反映的是「這個詞的意思用當地語言怎麼講」,不是「當地人實際會打進 Google 的字」,兩者
   常常不一樣(保留英文借詞不翻、用完全不同的框架、當地口語簡稱等)。正確做法是用 Serper
   把 `gl`/`hl` 設成目標市場語言直接查(joaillerie 用 `gl=fr&hl=fr`,Desk 用
   `gl=jp&hl=ja`),讓 `organic` 結果裡真實出現的當地語言字詞當關鍵字依據——這一步比任何
   翻譯都準確,因為是從真實排名結果反推出來的,不是語言轉換出來的。這條規則這個 session
   一直有照做(joaillerie/Desk 的關鍵字都是從 Serper 真實法文/日文結果來的),只是先前沒有
   寫成明文規則,補在這裡避免以後疏忽退化成翻譯猜測。
